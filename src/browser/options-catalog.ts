...
// Clear after bulk action: setSel([])`,
  ...
  // ── Async CRUD hooks ──────────────────────────────────────────────────────
  // Run `monkeytab docs crud-hooks` for the full story.
  {
    prop: 'rowKey',
    description: 'Resolve each row\'s id. Falls back to row.id, then an internal rec-N id.',
    category: 'data',
    type: 'string | ((row: Record<string, Value>) => string)',
    default: undefined,
    status: 'implemented',
    visibility: 'public',
    affects: 'Determines which id the grid reports to onCellSave / onRowDelete. Required when your rows have a DB-assigned id in a field other than `id`.',
    consumedBy: ['MonkeyTable'],
    example: `rowKey="dbId"
// or
rowKey={(row) => \`\${row.tenantId}:\${row.recordId}\`}`,
  },
  {
    prop: 'onRowCreate',
    description: 'Async hook invoked when a new row is added. Return the DB-assigned id; rejection removes the optimistic row.',
    category: 'data',
    type: '(draft: { fields: Record<string, Value> }) => Promise<{ id: string; fields?: Record<string, Value> }>',
    default: undefined,
    status: 'implemented',
    visibility: 'public',
    affects: 'Runs inside the optimistic-update lifecycle. The grid shows a pending row while awaiting; subsequent cell edits on that row are disabled until resolve. On resolve, the returned id replaces the temp id.',
    consumedBy: ['MonkeyTable', 'useCreateRecord'],
    example: `onRowCreate={async ({ fields }) => {
  const row = await api.insert(fields);
  return { id: row.id, fields: row };
}}`,
  },
  {
    prop: 'onCellSave',
    description: 'Async hook invoked when a single cell is saved. Awaited before the edit is considered durable; rejection rolls the cell back to oldValue.',
    category: 'data',
    type: '(rowId: string, fieldId: string, newValue: Value, oldValue: Value) => Promise<void>',
    default: undefined,
    status: 'implemented',
    visibility: 'public',
    affects: 'Runs inside the optimistic-update lifecycle. Cell shows new value immediately + saving border (if showCellSaveStatus) while awaiting. On reject, cell rolls back and onHookError fires.',
    consumedBy: ['MonkeyTable', 'useUpdateRecord'],
    example: `onCellSave={async (rowId, fieldId, newValue, oldValue) => {
  await api.patch(rowId, { [fieldId]: newValue });
}}`,
  },
  {
    prop: 'onRowDelete',
    description: 'Async hook invoked when a row is deleted. Awaited before the delete is considered durable; rejection restores the row.',
    category: 'data',
    type: '(rowId: string) => Promise<void>',
    default: undefined,
    status: 'implemented',
    visibility: 'public',
    affects: 'Row disappears immediately from the grid; on reject, the row is restored and onHookError fires.',
    consumedBy: ['MonkeyTable', 'useDeleteRecord'],
    example: `onRowDelete={async (rowId) => {
  await api.delete(rowId);
}}`,
  },
  {
    prop: 'onHookError',
    description: 'Fires on any CRUD hook rejection or required-field violation.',
    category: 'data',
    type: `(kind: 'create' | 'update' | 'delete', err: unknown) => void`,
    default: undefined,
    status: 'implemented',
    visibility: 'public',
    affects: 'Surface point for hook failures. Called with the mutation kind and the thrown error. Pair with errorToast for inline UI feedback, or wire your own toast system.',
    consumedBy: ['MonkeyTable'],
    example: `onHookError={(kind, err) => toast.error(\`[\${kind}] \${err instanceof Error ? err.message : String(err)}\`)}`,
  },
  {
    prop: 'errorToast',
    description: 'When true, MonkeyTable renders an inline error toast for 3s whenever onHookError fires.',
    category: 'data',
    type: 'boolean',
    default: false,
    status: 'implemented',
    visibility: 'public',
    affects: 'Small absolute-positioned toast inside the MonkeyTable root. Opt-in so host apps can wire their own toast library.',
    consumedBy: ['MonkeyTable'],
    example: 'errorToast',
  },
  {
    prop: 'showCellSaveStatus',
    description: 'When true, cells render a 2px left border while their onCellSave is in flight (blue) or after a reject (red for 1.5s).',
    category: 'data',
    type: 'boolean',
    default: false,
    status: 'implemented',
    visibility: 'public',
    affects: 'Per-cell saving indicator. Pending-create rows show a dashed blue border across every cell. Local to each cell — no prop drilling required.',
    consumedBy: ['GridCell'],
    example: 'showCellSaveStatus',
  },
  // ── Sorting ──────────────────────────────────────────────────────────────
  {
    prop: 'sortBy',
  ...
  // ── Toolbar ───────────────────────────────────────────────────────────────
  {
    ...
    affects: 'When false, hides the entire toolbar (search, filters, row height, add row, selection count). Also makes bulk delete and selectionActions unreachable — checkbox selection still works, but the badge that holds those buttons lives in the toolbar. Drive bulk operations via controlled selectedRowIds + onSelectionChange when the toolbar is hidden.',
    consumedBy: ['TableView'],
  ...
  {
    prop: 'textPopup',
    description: 'Global defaults for the Text column popup editor (multiline / richText / json).',
    category: 'advanced',
    type: '{ width?: number | string; minHeight?: number | string; maxHeight?: number | string }',
    default: undefined,
    status: 'implemented',
    visibility: 'public',
    affects: 'Provides default sizing for the TextEditor popup. Accepts pixels (number) or CSS length strings like "50vw"/"60vh". Per-column TextFieldOptions.popupWidth/popupMinHeight/popupMaxHeight override these. RichText columns also get a Write/Preview toggle, a B/I/Code/Link/Strike toolbar, and ⌘/Ctrl+B/I/E/K shortcuts.',
    consumedBy: ['MonkeyTable', 'TextEditor', 'TextPopupSizeProvider'],
    example: `textPopup={{ width: '50vw', minHeight: 240, maxHeight: '60vh' }}`,
  },
  {
    prop: 'onColumnRename',
    description: 'Fires after the user renames a column via the header menu or double-click.',
    category: 'advanced',
    type: '(fieldId: string, newLabel: string, oldLabel: string) => void',
    default: undefined,
    status: 'implemented',
    visibility: 'public',
    affects: 'Gates the "Rename column" header-menu item and the double-click-to-rename affordance. Without this callback, neither UI entry renders, because the consumer has no way to observe the change — the internal rename would silently revert on the next `columns` prop change.',
    consumedBy: ['MonkeyTable', 'TableView', 'GridHeader'],
    example: `onColumnRename={(fieldId, newLabel) => setColumns(cols => cols.map(c => c.id === fieldId ? { ...c, label: newLabel } : c))}`,
  },
  {
    prop: 'onColumnDelete',
    description: 'Fires after the user deletes a column via the header menu.',
    category: 'advanced',
    type: '(fieldId: string) => void',
    default: undefined,
    status: 'implemented',
    visibility: 'public',
    affects: 'Gates the "Delete column" header-menu item. Without this callback (and `allowDeleteField: true`), the menu entry is hidden.',
    consumedBy: ['MonkeyTable', 'TableView', 'GridHeader'],
    example: `onColumnDelete={(fieldId) => setColumns(cols => cols.filter(c => c.id !== fieldId))}`,
  },
  {
    prop: 'onColumnCreate',
    description: 'Fires after the user adds a column (via the "+" button, ghost cell, or formula builder).',
    category: 'advanced',
    type: '(field: { id: string; label: string; type: FieldType; options?: FieldOptions }) => void',
    default: undefined,
    status: 'implemented',
    visibility: 'public',
    affects: 'Gates the "+" add-column button and the ghost-column create path. Consumer receives the internally-generated id and can append the field to their `columns` prop.',
    consumedBy: ['MonkeyTable', 'TableView', 'Grid', 'GridHeader'],
    example: `onColumnCreate={(field) => setColumns(cols => [...cols, field])}`,
  },
  {
    prop: 'onColumnChangeType',
    description: 'Fires after the user changes a column\u2019s field type via the header menu.',
    category: 'advanced',
    type: '(fieldId: string, newType: FieldType) => void',
    default: undefined,
    status: 'implemented',
    visibility: 'public',
    affects: 'Gates the "Change type" header-menu item. Conversion validation still runs through the adapter (onColumnValidateType) before the callback fires.',
    consumedBy: ['MonkeyTable', 'TableView', 'GridHeader'],
    example: `onColumnChangeType={(fieldId, newType) => setColumns(cols => cols.map(c => c.id === fieldId ? { ...c, type: newType } : c))}`,
  },
  {
    prop: 'onColumnUpdateOptions',
    description: 'Fires after the user edits a column\u2019s type-specific options (select choices, number format, etc.).',
    category: 'advanced',
    type: '(fieldId: string, options: FieldOptions) => void',
    default: undefined,
    status: 'implemented',
    visibility: 'public',
    affects: 'Gates the "Customize" header-menu item. Without this callback, the options panel cannot be opened because edits would not persist through your `columns` prop.',
    consumedBy: ['MonkeyTable', 'TableView', 'GridHeader'],
    example: `onColumnUpdateOptions={(fieldId, options) => setColumns(cols => cols.map(c => c.id === fieldId ? { ...c, options } : c))}`,
  },
];
